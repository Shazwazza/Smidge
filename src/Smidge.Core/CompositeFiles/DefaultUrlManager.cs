        ...
        public IEnumerable<FileSetUrl> GetUrls(IEnumerable<IWebFile> dependencies, string fileExtension, string cacheBusterValue)
        {
            ...
            while (remaining.Count > 0)
            {
                   ...
                   + 10 >= _options.MaxUrlLength)
                {
                    ...
                    files.Add(new FileSetUrl { Key = _hasher.Hash(output), Url = GetCompositeUrl(output, fileExtension, cacheBusterValue) });
